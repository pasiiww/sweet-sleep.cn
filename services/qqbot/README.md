...
- 正常时生成2–5组标准名检索词、合并去重检索内容，再直接回复客服回答，不附加引用列表；模型未配置、关闭、余额不足、密钥无效或超时时只回复最相关文档片段。资料无命中或模型判定依据不足时转人工。
...
按知识库＋群/私聊＋发送者隔离，使用最近30分钟、最多10轮且12000字符内的完整问答。历史保存在已有 `seen.db` 的 `dialogue` 表，发送成功才写入；同一会话串行处理，重启后仍能继续。已过期记录在历史读写时清理，不会被发送给模型；系统错误、帮助、身份和未被艾特的群消息不入历史。

发送 `/新对话` 或 `/清空上下文` 可清除自己的当前会话（群内仍须艾特）。不同群、不同用户及私聊不会共享历史。别名表在知识库后台维护，程序强制加入别名说明；PE1 仅接收当前问题及对应别名；PE2 接收检索资料、别名和此前问答。检索词输出异常时仍通过本地检索进入 PE2，回答不可用时才回退原文。机器人只把自身维护的历史传入 `/answer`，不接受QQ消息指定其他用户的会话。
...
咨询额度按用户 OpenID 统一存储于知识服务，群聊、私聊、不同群及知识库共用每日20次（北京时间零点重置）；第21次起在检索与模型调用前直接回复额度用完。帮助、身份和清空上下文命令不计次，额度不足回复不写入多轮上下文。后台管理预览无用户身份，不占顾客额度。PE1 与 PE2 均携带同一会话最近30分钟、最多10轮历史；固定 system 与历史位于前缀，动态别名和本次内容位于最后一条 user 消息。Trace 展示额度和服务商返回的缓存命中 tokens。
