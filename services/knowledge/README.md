...
客服与 Agent 的知识搜索先用 FTS 扩大候选，再按当前问题中的商品、所问属性、批次和问句相似度排序；明确属于其他商品或旧批次的 QA 不送给回答模型。直接回答问题的 QA 优先于泛用文档。离线历史咨询测评见 [`evals/README.md`](evals/README.md)。
...
- 一次模型调用完成相关性判断和事实提取。闲聊、猜测、无依据信息不入库。问题必须写明商品或活动主体及必要的批次、款式或条件；无法确定指代的事实跳过。写入前对比现有 QA，同一实体、属性和稳定适用范围的较新消息在原 QA 上更新，保留旧内容的来源历史，不新增重复条目；较旧消息或处理期间被人工修改的记录跳过。低确定度的已有知识更新只在学习 Trace 中提出，待人工核对。
