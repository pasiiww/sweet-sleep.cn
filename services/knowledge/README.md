...
后台「实体别名」按知识库维护名称表，每行 `标准名 | 别名1 | 别名2`，也支持空格分隔（多词名称用竖线）。保存整表完成增删改查，校验名称归属冲突；最多200实体、每实体20别名、每名称80字符。`GET/PUT /bases/{id}/entities` 仅管理员可访问，PUT 格式 `{"items":[{"name":"凯伊","aliases":["kei"]}]}`，空列表清空。随知识库删除，独立于业务文档和 Embedding 设置。
