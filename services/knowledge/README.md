...
`POST /answer` 可携带 `history`，是最多10轮、总计12000字符的完整 user/assistant 消息对，禁止 system 消息，按时间顺序放在本次提问前。仅 PE2 回答阶段收到历史，历史仅用于指代与交流，不作为店铺事实依据。响应增加 `alias_context`、`matched_aliases` 和 `history_turns`，后台预览展示这些信息。该 API 不存储会话，30分钟期限由 QQ 客户端/后台预览维护；外部调用者负责历史选择。
...
表情包频率：PE2 优先用文字回答咨询，仅偶尔附图。QQ 按同一用户当前会话最近 30 分钟内的实际发送结果持久化 `sticker_state`；上一条成功附图时传入 `previous_sticker_sent=true`，PE2 接收 `sticker_allowed=false`，服务端与 QQ 发送端均禁止本轮附图。纯文字回复成功后才恢复可选，图片失败不计为成功附图。后台预览使用相同间隔规则，Trace 展示频率控制原因。
