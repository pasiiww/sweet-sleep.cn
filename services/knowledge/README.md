...
模型提示词要求每组保留实体或可靠别名，优先原始意图；回答提示词要求区分总价、定金和尾款。分组只支持 keyword 模式，vector/hybrid 继续使用原来的 query 接口，组合传入返回400，避免默默放宽匹配。关键词依赖资料中的实际写法，别名和不同字段应放在不同组；跨分段的实体信息建议写在文档标题中。
