...
Copy `mcp_server.py` to `/opt/sweet-knowledge/mcp_server.py`. No service restart is required: the MCP client launches one process per connection over SSH. Verify the client can initialize and call `list_knowledge_bases` after connecting.
