...
class QATests(unittest.TestCase):
    ...
    def test_mixed_retrieval_and_model_reference_contract(self):
        ...
        self.assertEqual([{k:v for k,v in r.items() if k!='updated_at'} for r in payload['retrieved_qa']],[{'question':'凯伊价格定金','answer':'定金20元。'}])
        self.assertTrue(payload['retrieved_qa'][0]['updated_at'])
        self.assertEqual([{k:v for k,v in r.items() if k!='updated_at'} for r in payload['retrieved_documents']],[{'title':'凯伊','content':'价格100元。'}])
        self.assertTrue(payload['retrieved_documents'][0]['updated_at'])
