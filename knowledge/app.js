...
function productCharacters(){return [...new Set($('product-form').elements.character.value.split(/[\n,，、]+/).map(v=>v.trim()).filter(Boolean))];}
function productVariant(row={}){
  const characters=productCharacters(), types=[...$('product-types-rows').querySelectorAll('[data-key="name"]')].map(el=>el.value.trim()).filter(Boolean);
  if(!characters.length||!types.length){toast('先填写角色列表和制品类型');return;}
  const div=document.createElement('div');div.className='product-variant';
  const options=(values,selected)=>[...new Set([...values,...(selected?[selected]:[])])].map(v=>`<option ${v===selected?'selected':''}>${esc(v)}</option>`).join('');
  div.innerHTML=`<label>角色<select data-key="character">${options(characters,row.character)}</select></label><label>类型<select data-key="type">${options(types,row.type)}</select></label><label>单独参考价<input data-key="price" inputmode="decimal" maxlength="30" placeholder="参考类型价" value="${esc(row.price||'')}"></label><label>售卖情况<select data-key="status">${options(['待确认','在售','售罄','不售卖'],row.status||'待确认')}</select></label><button type="button">移除规格</button>`;
  div.querySelector('button').onclick=()=>div.remove();$('product-variants-rows').append(div);
}
$('product-variant-add').onclick=()=>productVariant();
...
function editProduct(row=null){
  ...
  if(row)$('product-form').elements.character.value=(row.characters||[row.character]).join('\n');
  ...
  $('product-variants-rows').innerHTML='';(row?.variants||[]).forEach(productVariant);
...
$('product-form').onsubmit=e=>{
  ...
  busy(e.submitter,async()=>{
    ...
    data.characters=productCharacters();
    data.variants=[...$('product-variants-rows').children].map(r=>Object.fromEntries([...r.querySelectorAll('[data-key]')].map(el=>[el.dataset.key,el.value.trim()])));
...
async function loadAnswerSettings() {
  ...
  const maintenance=await api('private-maintenance-settings');
  $('private-maintenance-openids').value=maintenance.openids.join('\n');
...
async function loadStickers() {
 const {items}=await api('stickers');
 $('sticker-empty').hidden=items.length>0;
 $('sticker-rows').innerHTML=items.map(s=>`<tr data-sticker="${s.id}"><td><img class="sticker-preview" src="${esc(s.url)}" alt="${esc(s.name)}" loading="lazy" referrerpolicy="no-referrer"></td><td><input aria-label="表情包名字" data-field="name" value="${esc(s.name)}" maxlength="60"></td><td><input aria-label="图片路径" data-field="path" value="${esc(s.path)}" maxlength="2000"><small>${esc(new Date(s.updated_at).toLocaleString())}</small></td><td><input aria-label="启用表情包" data-field="enabled" type="checkbox" ${s.enabled?'checked':''}></td><td><button data-action="save">保存</button><button data-action="delete">删除</button></td></tr>`).join('');
 $('sticker-rows').querySelectorAll('tr').forEach(row=>{
   const original=items.find(s=>String(s.id)===row.dataset.sticker);
   row.querySelectorAll('button').forEach(button=>button.onclick=()=>busy(button,async()=>{
     if(button.dataset.action==='delete') {if(!confirm('删除这个表情包？'))return;await api('stickers/'+original.id,'DELETE');}
     else await api('stickers/'+original.id,'PUT',{name:row.querySelector('[data-field=name]').value,path:row.querySelector('[data-field=path]').value,enabled:row.querySelector('[data-field=enabled]').checked,revision:original.revision});
     await loadStickers();toast('已保存');
   }));
 });
}
