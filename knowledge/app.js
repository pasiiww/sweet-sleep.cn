...
$('answer-preview-form').onsubmit = event => { event.preventDefault(); busy(event.submitter, async () => {
  ...
  try {
    ...
    previewTurns.set(kb, [...(previewTurns.get(kb) || []), {query, reply: result.answer, sticker:!!result.sticker, at: Date.now()}].slice(-10));
